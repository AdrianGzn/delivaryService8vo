...
func (h *LoginHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	err = h.DB.QueryRow(
		"SELECT id, name, password, role, address, establishmentName, establishmentAddress FROM users WHERE name = ?",
		loginReq.Name,
	).Scan(&user.ID, &user.Name, &user.Password, &user.Role, &user.Address, &user.EstablishmentName, &user.EstablishmentAddr)
...
func (h *LoginHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	establishmentName, _ := data["establishmentName"].(string)
	establishmentAddress, _ := data["establishmentAddress"].(string)

	log.Printf("Valores extraídos - Name: '%s', Password: '%s', Role: '%s', Address: '%s', EstablishmentName: '%s', EstablishmentAddress: '%s'",
		name, password, role, address, establishmentName, establishmentAddress)
	...
	log.Printf("Intentando insertar en BD: %s, %s, %s, %s, %s, %s", name, password, role, address, establishmentName, establishmentAddress)
	...
	response := models.User{
		...
		EstablishmentAddr: &establishmentAddress,
	}
